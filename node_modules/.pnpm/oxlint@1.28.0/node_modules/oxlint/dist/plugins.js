...
var require_eslint_visitor_keys = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	let types_1$7 = require_dist$1(), assert_1$4 = require_assert(), definition_1$5 = require_definition(), ID_1 = require_ID(), Reference_1$2 = require_Reference(), variable_1$1 = require_variable(), ScopeType_1$18 = require_ScopeType();
...
})), import_dist = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports) => {
...
function isGlobalReference(node) {
	...
	let variable = scopes[0].set.get(name);
...
function getTokensBetween(nodeOrToken1, nodeOrToken2, countOptions) {
	throw Error("`sourceCode.getTokensBetween` not implemented yet");
}
function getFirstTokenBetween(nodeOrToken1, nodeOrToken2, skipOptions) {
	throw Error("`sourceCode.getFirstTokenBetween` not implemented yet");
}
function getFirstTokensBetween(nodeOrToken1, nodeOrToken2, countOptions) {
	throw Error("`sourceCode.getFirstTokensBetween` not implemented yet");
}
function getLastTokenBetween(nodeOrToken1, nodeOrToken2, skipOptions) {
	throw Error("`sourceCode.getLastTokenBetween` not implemented yet");
}
function getLastTokensBetween(nodeOrToken1, nodeOrToken2, countOptions) {
...
function isSpaceBetween(nodeOrToken1, nodeOrToken2) {
	let range1 = nodeOrToken1.range, range2 = nodeOrToken2.range, start1 = range1[0], start2 = range2[0], gapStart, gapEnd;
	return start1 < start2 ? (gapStart = range1[1], gapEnd = start2) : (gapStart = range2[1], gapEnd = start1), gapStart >= gapEnd ? !1 : (sourceText === null && initSourceText(), WHITESPACE_REGEXP.test(sourceText.slice(gapStart, gapEnd)));
}
...
function resetSourceAndAst() {
	buffer = null, sourceText = null, ast = null, parserServices = null, resetBuffer(), resetLines(), resetScopeManager();
}
const SOURCE_CODE = Object.freeze({
	...
	isSpaceBetween
}), { defineProperty } = Object, LINE_BREAK_PATTERN = /\r\n|[\r\n\u2028\u2029]/gu, lines = [], lineStartOffsets = [0];
function initLines() {
...
function initSettings() {
	settings = JSON.parse(settingsJSON), deepFreezeSettings(settings);
}
function resetSettings() {
	settings = null, settingsJSON = null;
}
function deepFreezeSettings(obj) {
	if (!(typeof obj != "object" || !obj)) {
		if (isArray$2(obj)) for (let i = 0, len = obj.length; i < len; i++) deepFreezeSettings(obj[i]);
		else for (let key in obj) deepFreezeSettings(obj[key]);
		Object.freeze(obj);
	}
}
const { hasOwn, keys: ObjectKeys$3, freeze, assign: ObjectAssign, create: ObjectCreate } = Object, diagnostics = [];
...
const LANGUAGE_OPTIONS = freeze({
	...
	ecmaVersion: 2026,
	get parser() {
		throw Error("`context.languageOptions.parser` is not implemented yet.");
	},
	parserOptions: freeze({ get sourceType() {
		return ast === null && initAst(), ast.sourceType;
	} }),
	get globals() {
		return null;
	...
	get parserOptions() {
		throw Error("`context.parserOptions` is deprecated. Use `languageOptions` instead.");
	},
	get parserPath() {
		throw Error("`context.parserPath` is deprecated. No longer supported.");
	},
	getCwd() {
		throw Error("`context.getCwd` is deprecated. Use `cwd` instead.");
	},
	getFilename() {
		throw Error("`context.getFilename` is deprecated. Use `filename` instead.");
	},
	getPhysicalFilename() {
		throw Error("`context.getPhysicalFilename` is deprecated. Use `physicalFilename` instead.");
	},
	getSourceCode() {
		throw Error("`context.getSourceCode` is deprecated. Use `sourceCode` instead.");
	}
});
...
function reportImpl(diagnostic, ruleDetails) {
...
const ObjectKeys$2 = Object.keys, registeredPluginPaths = /* @__PURE__ */ new Set(), registeredRules = [], neverRunBeforeHook = () => !1, DEFAULT_OPTIONS = Object.freeze([]);
async function loadPlugin(path, packageName) {
	try {
		let res = await loadPluginImpl(path, packageName);
		return JSON.stringify({ Success: res });
	} catch (err) {
		return JSON.stringify({ Failure: getErrorMessage(err) });
	}
}
async function loadPluginImpl(path, packageName) {
	...
	let pluginName = plugin.meta?.name ?? packageName;
	if (!pluginName) throw TypeError("Plugin must have either meta.name or be loaded from an npm package with a name field in package.json");
	let offset = registeredRules.length, { rules } = plugin, ruleNames = ObjectKeys$2(rules), ruleNamesLen = ruleNames.length;
...
function lintFile(filePath$1, bufferId, buffer$1, ruleIds, settingsJSON$1) {
	try {
		return lintFileImpl(filePath$1, bufferId, buffer$1, ruleIds, settingsJSON$1), JSON.stringify({ Success: diagnostics });
	} catch (err) {
		return JSON.stringify({ Failure: getErrorMessage(err) });
	} finally {
...
function lintFileImpl(filePath$1, bufferId, buffer$1, ruleIds, settingsJSON$1) {
	...
	if (typeof filePath$1 != "string" || filePath$1.length === 0) throw Error("expected filePath to be a non-zero length string");
	...
	for (let i = 0, len = ruleIds.length; i < len; i++) {
		...
		let { rule, context } = ruleDetails, { visitor } = ruleDetails;
